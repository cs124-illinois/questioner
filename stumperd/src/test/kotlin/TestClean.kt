...
class TestClean : StringSpec() {
    override suspend fun beforeSpec(spec: Spec) {
        testCollections.reset()
    }
    init {
        "it should clean submissions correctly" {
            val results = pipeline(pipelineOptions.copy(limit = 1024, stepLimit = Steps.CLEAN.value)).toList()
            ...
            testCollections.doneCollection.successCount shouldBe results.count() - 1 - 81 - 10
            testCollections.doneCollection.failureCount shouldBe 1 + 81 + 10
